...
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5))    #each txn to have a minimum of 2 items and max of 5 items and sup = 0.01 means say out of 10000 txns, 2 items should be purchased atleast 100 times to be considered further for confidence
...
itemFrequencyPlot(Groceries, topN = 10, type='relative')    #relative to the total number of txns irrespective of whether or not the item was purchased in that txn or not. 
abline(h=0.15)
...
rules   #using confidence, 52% of the people who bought baking powder also bought whole milk
inspect (rules[1:5])
...
rules3 = apriori (data=Groceries, parameter=list (supp=0.002,conf = 0.8), appearance = list (default="lhs",rhs="whole milk"), control = list (verbose=F))    #defined at the time of creation what should come on rhs or we can create a subset of a rules which has been created earlier without information about what comes on the rhs
inspect(rules3[1:5])
inspect(rules3)
